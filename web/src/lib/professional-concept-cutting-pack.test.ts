...
describe("O1 cutting pack", () => {
  it("preserves all original concept/value content apart from version labels", () => {
    const baselineContent = JSON.parse(JSON.stringify({ concepts: o2Pack.concepts.slice(0, 5), canonicalValues: o2Pack.canonicalValues }), (key, value) => ["specificationVersion", "semanticVersion"].includes(key) ? undefined : value);
  ...
  it("reuses existing English display labels verbatim without using them as identities", () => {
    for (const value of pack.canonicalValues.filter(v => v.status === "AMBIGUOUS")) {

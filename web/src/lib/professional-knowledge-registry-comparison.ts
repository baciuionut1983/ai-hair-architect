...
function evaluateParameters(unit: ProfessionalKnowledgeUnit, skill: ProfessionalSkillDefinitionRecord): { evidence: KnowledgeParameterEvidence[]; conflicts: KnowledgeParameterConflict[] } {
  ...
  for (const parameter of parameters) {
    ...
    // own "reflects what professional review ADDED" scope).
    if (!field || field.source !== "PROFESSIONAL_INPUT" || typeof field.value !== "string") continue;
    // Only a FIXED parameter (exactly one allowed value) counts as
    // meaningful comparison evidence -- see professional-learning-reviewed-
    // comparison.ts's own identical reasoning.
    if (!parameter.allowedValues || parameter.allowedValues.length !== 1) continue;

    const fieldNumber = extractLeadingNumber(field.value);
    const allowed = parameter.allowedValues[0];
    ...
    const exactMatch = fieldNumber === null && typeof allowed === "string" && allowed.toLowerCase() === field.value.toLowerCase();

    if (numericMatch || exactMatch) {
      evidence.push({ skillId: skill.skillId, fieldName, parameterName: parameter.name });
    } else if (fieldNumber !== null && allowedNumber !== null) {
      // A genuine, comparable numeric value that does NOT match the
      // skill's own fixed allowed value is a real structural conflict
      // (Section 36) -- never silently ignored, never auto-resolved.
      conflicts.push({ skillId: skill.skillId, fieldName, parameterName: parameter.name, existingAllowedValue: allowed, proposedValue: field.value });
    }
  }
...
export function compareKnowledgeUnitAgainstRegistry(
  ...
  for (const skill of registry) {
    const { evidence: parameterEvidence, conflicts } = evaluateParameters(unit, skill);

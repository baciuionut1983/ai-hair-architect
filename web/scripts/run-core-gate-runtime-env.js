...
function run() {
  ...
  const result = envGate.validateRuntimeProductionEnvironment(process.env);

  if (!result.ok) {
    const formatted = envGate.formatValidationIssues(result.issues);
    console.error("[CORE_GATE][RUNTIME_ENV] Validation failed.");
    if (formatted) {
  ...
  console.log("[CORE_GATE][RUNTIME_ENV] Validation passed.");
}

...
suite("m13 backup verification integration", () => {
  ...
  it("returns a dedicated storage-metadata-missing diagnostic for a legacy S3-backed asset (storagePath \"pending\"), not the generic unsafe-path error (M33 GO-3)", async () => {
    const ownerUserId = randomUUID();
    const backupId = `c${Date.now().toString(36)}${Math.random().toString(16).slice(2, 14)}`;
    const artifact: BackupV13Artifact = {
      schemaVersion: BACKUP_V13_SCHEMA_VERSION,
  ...
  it("documents current behavior: verify does not support m15.v1 (fails closed, unlike restore-preview which does support it) -- m15.v1 has zero production creation callers so no real backup is affected", async () => {
    const ownerUserId = randomUUID();
    const backupId = `c${Date.now().toString(36)}${Math.random().toString(16).slice(2, 14)}`;
    ...
    await prisma.opsBackupSnapshot.create({
      data: {
        ...
        ownerUserId,
        label: "m15v1-probe",
        snapshotJson: { schemaVersion: "m15.v1" },
        checksum: "irrelevant",
        checksumAlgorithm: "sha256",
